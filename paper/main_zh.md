...
另一类方法将模型输出置于更接近规划与控制边界的位置。C-TRAIL 将常识推理引入轨迹规划 [5]；策略迁移方法使用语言模型解析任务描述并选择可复用驾驶策略 [6]；生成式驾驶方法学习预测性场景表示或未来轨迹，以服务下游规划 [7]--[10]。这些方法提高了模型输出的运行相关性，也使时间对齐更为重要：提案越接近执行，能够修正早期状态所生成动作的下游环节越少。上述研究主要关注如何生成有效的驾驶提案或表示。RGD 处理与之互补的系统问题，即延迟提案如何获得执行权限。它不替代底层推理器、规划器或学习表示，而是提供统一的释放接口，在当前状态中重新解释返回输出，并与快速控制器已经提供的动作进行比较。
...
选择性计算与元推理密切相关，后者在预期决策改进与计算代价之间进行权衡。限时与 anytime 方法在有限预算下调度推理 [15]--[18]，并发规划允许执行器在规划器计算期间继续动作 [19]。这些原理为查询准入提供依据，但查询时价值只是对未来机会的估计。在闭环驾驶中，推理期间交通继续演化，快速控制器也持续动作；支撑请求的替代方案可能在响应到达前消失、变得冗余或被取代。RGD 将选择性计算从调用扩展到权限管理。准入门控控制是否值得启动慢推理，释放门控控制是否值得应用其实际输出。R-VoD 则度量释放状态中剩余的纠正机会，而非查询状态下推断的预期价值。这种分离避免将成功发出的请求视为无条件的未来命令。
...
RGD 将慢推理器调用分解为两个连续的权限决策：查询准入与释放验证。准入阶段判断当前场景是否需要审议，以及预期响应时段内能否支持可行替代动作；释放验证在响应交付时的观测状态中评估返回提案。快速控制器在整个推理期间始终为默认路径，并在任一阶段不授予慢路径权限时继续控制。

若释放状态 $s_r$ 存在至少一个可行有效替代动作，且其相对快速有效动作的匹配推演优势达到 R-VoD 裕度，则 $s_r$ 为**可恢复状态**。R-VoD 通过匹配推演在离线阶段量化该纠正机会。只有当返回提案的映射命令通过当前状态可行性检查、不同于映射后的快速命令，并满足提案特定的恢复代价裕度时，该提案才在释放时具有**可执行性**。最终安全投影仍可能将已授权的慢命令折叠为与快速命令相同的可执行动作；因此，R-VoD 与释放验证相互补充但不可互换。

![图 1 RGD 系统架构](figures/sturcture_new.png)

*图 1. RGD 系统架构。实线箭头表示在线控制路径，虚线箭头表示离线 R-VoD 诊断分支。memory 模块属于慢推理器输入上下文，而非在线门控。*

图 1 展示两个在线门控与离线 R-VoD 诊断分支。快速控制器在每个控制周期保持活动；响应缺失、映射失败或释放被拒绝时，系统均保留快速命令。两个分支的输出先进入同一映射命令空间，再经过共享的可行性与代价接口，从而保证语义等价的表达无论采用何种推理器措辞，均解析为相同机动。

### A. 查询与释放过程

在帧 $t$，本车观测状态 $s_t$。快速策略产生动作，并由共享映射器转换为 $a_t^f$。两个分支共享该映射器与最终安全投影 $\Phi(s,a)$，后者在状态 $s$ 中将命令映射为可执行动作。每个映射命令 $a\in\mathcal A(s)$ 均包含机动原语与目标速度；若两个表达产生相同的二元组，则视为等价。机动分量包括向左变道、保持车道、向右变道、加速与减速，所有机动均通过同一仿真器接口解析。响应解析器 $\psi(y)$ 对结构化标识符或标准词元进行规范化，$\mathcal M(s,y)$ 则在 $\mathcal A(s)$ 中构造相应命令；当机动或目标速度在状态 $s$ 中无效时，该映射未定义。这一过程属于当前状态语义映射，而非轨迹重规划。

返回提案在观测到的释放状态 $s_r$ 中重新映射。查询时可行的变道或速度命令在释放时可能已不再有效；任何映射或可行性失败都会使系统保留 $a_r^f$。

时延估计仅用于准入。实际释放帧 $r$ 是响应首次可用的控制帧，返回命令与快速命令均在观测状态 $s_r$ 中评估。回合终止前未到达的响应将被丢弃。
...
由于 $a_{\mathrm{eff}}^f\in\mathcal C(s)$，R-VoD 按构造非负；当不存在优于快速延续的替代动作时，其值为零。当 $\mathrm{R\text{-}VoD}(s)\geq\epsilon_{\mathrm R}$ 时，该状态被标记为**纠正状态**，其中 $\epsilon_{\mathrm R}=0.02$ 是固定的效用尺度裕度。尽管 $\epsilon_{\mathrm R}$ 与在线代价尺度裕度 $\delta$ 的数值相同，二者相互独立；R-VoD 不进入在线门控。

通过仅改变第一个有效动作，并令后续轨迹由共同快速策略产生，R-VoD 将释放时的纠正机会与慢推理器响应的下游变化分离。该构造直接对应第 IV 节中的纠正状态消融与匹配提案释放分析。

## IV. 实验与讨论

### A. 实验设置

本文在 highway-env [28] 与 MetaDrive [29] 闭环仿真器上评估 RGD，默认慢推理器为 Qwen3-8B [30]。主要 highway-env 实验以 10 Hz 运行，每回合持续 30 s。每项比较中的所有方法共享种子队列、动作模式、快速控制器、慢推理器接口、请求预算与评估规则。门控校准、主要评估与机制分析采用互不重叠的队列。配对资源分配研究使用 30 个种子与 6 种策略，共形成 180 个策略--种子单元。每项组件研究使用独立的 20 个种子队列；六条件消融包含 120 个配对条件--种子单元。

仿真器种子是实验单元与 bootstrap 单元。本文基于 20,000 次重采样报告 95% 百分位 bootstrap 区间。二元配对端点使用精确 McNemar 检验，连续端点使用配对符号翻转检验；每个端点内预先规定的 5 项 RGD--基线对比采用 Holm 校正，推断性讨论聚焦两个请求密集型比较对象。R-VoD 在释放后通过匹配首动作推演计算。门控判据与纠正裕度在校准队列上确定，服务时延层由单独的服务标定固定；三者在报告的评估中均保持冻结。固定在线设置为 $(\lambda_L,\lambda_A,\lambda_H,\lambda_N)=(0.05,0.55,0.10,0.20)$、$T_A=0.10$、$\kappa_c=0.55$ 与 $\rho=0$；1.7 s 预测时延是相对 1.572 s 服务时延中位数选取的最近保守预设层。分析覆盖六个方面：已发表设置下的性能背景、时延扫描下的准入选择性、释放门控验证、门控判据消融、推理器替换与跨场景迁移。门控阈值在留出的校准队列（种子 2000--2039）上，跨 0.7、1.7 与 2.7 s 三个时延层确定；选择目标是在最大化纠正状态裕度的同时，最小化每个纠正状态对应的请求数。

配对资源分配队列提供主要的资源与任务端点比较。独立的受控时延队列支持时序、验证与投影的事件级分析。为隔离释放验证，匹配审计固定 30 个配对种子上由 RGD 自然发出的 11 条提案；释放验证与无验证分支接收相同提案和释放时序。每个投影后不同动作均从其匹配的在线释放快照出发，通过式（7）的匹配推演效用与 Fast 比较。

![图 2 highway-env 与 MetaDrive 的代表性场景](figures/fig_simulator_scenes.png)

*图 2. highway-env 与 MetaDrive 的代表性场景。*

图 2 给出 highway-env 与 MetaDrive 的代表性场景。上排包含高速公路、汇入、环岛与交叉口几何，下排为对应的 MetaDrive 重建。这些场景在不改变动作契约的条件下，使同一查询--释放接口覆盖车道跟随、汇入冲突、环形交通与交叉口穿越。

### B. 已发表设置下的性能背景

在 PADriver [31] 的环境设置下，表 I 将 RGD 置于已报告方法的运行范围内。RGD 在 30 次运行中完成 29 次，行驶距离为 611 m、速度为 73.60 km/h，单帧运行时间为 37.6 ms，且无需针对评估环境进行专门训练。依照来源论文定义，Saf. 表示安全距离保持率，Kep. 表示车道保持率；二者均在 $[0,1]$ 内报告，数值越大表示相应事件发生频率越高。RGD 在该比较中获得最佳距离、速度、完成次数与运行时间。相较 PADriver--Normal，其距离增加 8 m，速度提高 1.13 km/h，成功运行增加 4 次，单帧运行时间则由 1.4 s 降至 0.038 s。合取式准入门控避免常规慢路径调用，提案等待或被拒绝时仍由快速路径控制车辆。这种职责分离无需在每个控制步执行慢推理，并保留纠正性推理入口，从而形成表中最强的行驶进度、完成次数与运行时间组合。PADriver 在其专用个性化策略目标下获得最高的 Saf. 与 Kep.。

Hu 等人 [27] 在 highway-env 中将成功定义为连续 30 帧无碰撞，其比较覆盖 4L/2.0、4L/2.5 与 5L/3.0，每种设置使用 10 个不同种子。本文采用相同交通设置与成功判据评估 RGD，RGD 运行中不人为增加响应时延。图 3 保留来源论文的三组基线结果并加入 RGD，表 II 给出相应运行指标。

![图 3 Hu 等人的三种 highway-env 交通设置下的成功率](figures/fig3_hu_protocol_comparison_compact.png)

*图 3. Hu 等人三种 highway-env 交通设置下的成功率。*

RGD 在三个设置中分别达到 100%、100% 与 80% 成功率，较每个设置中的最强已发表结果分别提高 31.4、43.8 与 46.9 个百分点。表 II 表明，该增益仅对应每回合 1.0、1.0 与 0.8 次慢路径请求，且三种设置下的单帧运行时间均低于 66 ms。最大优势出现在 5L/3.0；在该设置中，车道数与交通密度同时增加，使查询状态中的动作更容易在释放前失去支持。RGD 在两个边界处理这一问题：合取式准入将慢推理保留给具有动作支持的替代方案，当前状态验证则在间隙闭合或优势消失后拒绝返回机动，快速路径覆盖这些稀疏请求之间的控制周期。这种释放感知分工说明了在已发表基线随交通复杂度下降时，RGD 仍能维持较大成功率优势的原因。表 II 报告安全条件限定的速度，对失败回合赋值为零，并在每种设置的 10 个回合上计算平均请求数。
...
| 方法 | 距离（m） | 速度（km/h） | Saf. | Kep. | 完成（/30） | 时间（s/frame） |
|---|---:|---:|---:|---:|---:|---:|
...
1.7 s 实验考察交通状态演化后 RGD 如何处理慢响应。快速控制器、慢推理器、请求预算、种子队列与可行性接口保持固定。图 6 报告响应验证阶段与相应释放决策。每个返回提案到达的状态均晚于发起请求的状态，RGD 仅在其通过全部释放检查后才予以应用。

#### 4. 响应验证与组件效应

响应追踪记录每个请求从准入、响应验证、当前状态映射直至执行的过程。在相同的 1.7 s 附加时延下，逐项移除消融变体使用独立的 20 个种子队列；每个变体只移除一项准入判据，释放过程保持不变。查询准入后共有 143 个有效响应返回，其中 138 个在回合终止前到达释放阶段。当前状态可行性、动作支持与可接受代价检查保留 28 个响应；命令差异性保留 9 个；固定恢复代价裕度进一步保留 6 个。共享投影 $\Phi$ 将其中 4 个命令映射为与 Fast 相同的可执行动作，最终仅有 2 个动作能够改变执行。图 6（b）从全部 28 个状态有效结果给出相同分解：22 个未达到 0.02 裕度，4 个通过裕度但在投影后与 Fast 匹配，2 个保持不同。$143\!\rightarrow\!138\!\rightarrow\!28\!\rightarrow\!9\!\rightarrow\!6\!\rightarrow\!2$ 是该释放实验的核心结果。有效响应不会仅因到达而获得权限，143 个有效返回中有 141 个不改变最终快速路径动作。该追踪揭示了释放门控的主要价值：它将大量模型输出压缩为少量执行变化，并在延迟提案过期、冗余或优势不足时维持快速命令。

![图 6 1.7 s 附加时延下的响应验证流程](figures/fig2_v13_main_evidence_rev20260808.png)

*图 6. 1.7 s 附加时延下的响应验证。（a）从有效响应返回到投影后执行的阶段性留存；（b）28 个状态有效结果按恢复代价优势分层，虚线为固定 0.02 裕度，“最终匹配”表示投影后与快速动作相同。*
...
*图 7. 20 个种子上的准入判据逐项移除消融。（a）请求总数；（b）相对完整 RGD 的额外释放状态数；（c）R-VoD 纠正状态率差值（消融设置减去完整 RGD）及 95% 种子 bootstrap 区间。*

#### 5. 完整门控与投影分析

前述消融在自然请求流下检验准入判据作用，本分析则重放共享提案库，以在不受调度差异影响的条件下分别考察门控与投影因素。六个条件分别为完整 RGD、移除 $L$、移除 $A$、移除 $H$、移除 $N$，以及同时移除 $H$ 与 $N$。所有条件在固定 1.7 s 时延下，针对 20 个留出种子重放相同的门控独立提案源。动作映射器、释放规则、安全投影、预算与冷却时间保持不变，因此每项比较只移除指定的准入谓词。共享提案库由不读取所评估门控的 Qwen3-8B Always Slow 源冻结而成。每条记录绑定源帧、原始动作、响应结果、实测时延与响应哈希；所有条件重放同一提案库，并在每帧重新计算 Fast 动作。门控决策在向各条件暴露提案字段前完成。

图 8（a）追踪 70 个同步候选经过串行准入阶段的过程。完整 RGD 在 $L$ 后保留 49 个候选，在 $A$ 后保留 40 个，在 $H$ 后保留 8 个，在 $N$ 后不再保留候选。按阶段计算，$L$ 移除提案库的 30.0%，$A$ 移除余下候选的 18.4%，$H$ 移除 80.0%，$N$ 移除最后 8 个候选。单独移除 $L$ 或 $A$ 会改变上游轨迹，但由于下游谓词仍然有效，完整门控后仍为零。移除 $H$、移除 $N$ 或同时移除二者后，完整门控分别保留 1、8 与 40 个候选。图 8（b）将该留存转化为服务负载：三个条件分别发出 1、8 与 40 次请求，其中 1、5 与 26 次到达释放，0、3 与 14 次超时。因此，$H/N$ 组合阻止了 40 次请求涌入执行器。移除 $N$ 与同时移除 $H/N$ 还分别产生 4 个与 16 个不同于 Fast 的映射提案，但共享投影将其全部映射为与快速动作等价的可执行动作，120 个配对条件--种子单元的任务端点保持不变。完整门控由此去除了在该冻结提案库中不能产生不同执行动作的工作量。
...
在独立的 30 回合替换队列上，本文使用六种替代模型替换慢推理器，同时保持 RGD 门控、动作模式与回退路径不变。表 III 显示，有效响应数为 112--138，相对最小值变化 23%，但全部七个模型均完成 29/30 个回合。平均距离仅跨越 4.27 m，即 599.12--603.39 m。两个 Qwen3 变体返回最多有效响应，GPT-5.6-sol 返回最少，却取得最佳平均距离。因此，在 RGD 下，响应数量并不能直接代表闭环行驶进度。
...
表 IV 报告六种车道数/密度配置下的平均距离与成功次数，且未重新调整门控。在低密度（2.0）下，成功次数达到 28--30，平均距离为 599--629 m。在高密度（3.0）下，成功次数为 20--23，平均距离为 429--478 m；密集交通会缩小可行间隙并提高交互代价。RGD 在全部六种设置中使用相同的准入与释放参数。
...
依据 Wang 等人 [6] 的场景与指标定义，表 V 报告三个 MetaDrive 复合危险场景中的无碰撞安全率与平均速度。场景 A 引入随机行人横穿，场景 B 为存在随机冲突车流的无控制交叉口，场景 C 同时包含两类危险。RGD 在场景 A 中达到全表最佳的 100% 无碰撞安全率，并在场景 C 中以 6.9 m/s 取得最高平均速度。

这两项场景级优势对应 RGD 门控的不同作用。在场景 A 中，当前状态重映射可阻止延迟命令在行人运动已经改变横穿冲突后继续拥有权限。在场景 C 中，合取式准入将慢推理保留给存在动作支持的恢复机动；释放比较移除过期或冗余提案，快速路径则在危险事件之间维持行驶进度。ASR-RL 的其他场景--指标优势来自预训练策略库与场景风险策略迁移。RGD 使用统一门控配置，无需场景特定训练或重新调参，仍取得场景 A 安全率与场景 C 速度最优，说明释放感知权限管理为跨场景性能提供了与策略迁移互补的实现路径。

### G. 讨论

RGD 通过将慢推理视为受释放条件约束的提案，而非持续命令来源，处理查询状态与释放状态之间的时间失配。其核心价值在于将计算准入与执行权限分离：查询门控将审议集中于存在动作支持和纠正机会的状态，当前状态释放验证则防止过期、冗余或优势不足的提案替代快速控制。该分离方式在保持闭环控制连续性的同时，使审议仅在返回结果仍然有效时介入。共享动作接口与恢复代价比较还为所评估的推理器和交通条件提供一致的权限契约，R-VoD 则独立度量释放状态中可用的纠正机会。

*局限性与未来工作。* 当前评估采用离散高层命令、结构化推理器输出与受控仿真时延。未来工作将通过时延感知重规划，把 RGD 扩展到连续轨迹提案与随机服务时延，并在硬件在环和实车环境中评估释放契约。另一项研究方向是在交通分布变化下自适应标定裕度，同时保留明确的快速路径回退与当前状态验证。
