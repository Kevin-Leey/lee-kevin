...
def resolve_llm_endpoint(cfg: Dict[str, Any]) -> Tuple[str, str, str, str]:
    ...
    """
    provider = str(cfg.get("LLM_PROVIDER", "siliconflow") or "siliconflow").strip().lower()
    if provider in {"tokenplan", "mimo", "xiaomi-mimo"}:
        return (
            "tokenplan",
            _required_text(cfg, "TOKENPLAN_CHAT_MODEL"),
            _required_text(cfg, "TOKENPLAN_API_KEY"),
            _first_text(cfg.get("TOKENPLAN_BASE_URL"), "https://token-plan-cn.xiaomimimo.com/v1").rstrip("/"),
        )
    if provider in {"bizdecipher", "openai", "openai_compatible", "openai-compatible"}:
        ...
        if not model or not api_key:
            raise ValueError("openai-compatible provider requires OPENAI_CHAT_MODEL/OPENAI_API_KEY (or BIZDECIPHER_*)")
        return "openai_compatible", model, api_key, base_url

    # default: siliconflow
    return (
        "siliconflow",
        _required_text(cfg, "SILICONFLOW_CHAT_MODEL"),
        _required_text(cfg, "SILICONFLOW_API_KEY"),
        _required_text(cfg, "SILICONFLOW_BASE_URL").rstrip("/"),
    )


def setup_api(cfg: Dict) -> None:
    provider, model, api_key, base_url = resolve_llm_endpoint(cfg)
    timeout_s = max(0.1, float(cfg.get("QWEN_REQUEST_TIMEOUT_S", 7.0) or 7.0))
